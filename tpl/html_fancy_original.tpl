...
<%def name="host_kernelfacts(host)">
  % if len(host.get('kernel_facts', {}).items()) != 0:
    <h4>Kernel Facts</h4>
    <div>
    <table>
    <% 
    ...
    cont_cont=1;
    %>
    % for x in range(0, (len(host.get('kernel_facts')['stdout_lines']))/2):
        <tr><th>${host.get('kernel_facts')['stdout_lines'][cont_name]}</th><td>${host.get('kernel_facts')['stdout_lines'][cont_cont]}</td></tr>
        <% 
    ...
    % for y in range(cont_name, len(host.get('kernel_facts')['stdout_lines'])):
        <tr>
        % if host.get('kernel_facts')['stdout_lines'][cont_lim] == 'limits':
        <th>${host.get('kernel_facts')['stdout_lines'][cont_lim]}</th>
        % else:
        <td>${host.get('kernel_facts')['stdout_lines'][cont_lim]}</td>
        % endif
        </tr>
        <% cont_lim=cont_lim+1 %>
    % endfor
...
<%def name="host_databases(host)">
  % if len(host.get('databases', {}).items()) > 0 and len(host.get('databases', {})['stdout_lines']) > 0 :
    <h4>DataBases</h4>
    <div>
    <ul>    
      % for x in range(0, (len(host.get('databases')['stdout_lines']))):
          <li>${host.get('databases')['stdout_lines'][x]}</li>
      % endfor
     </ul>  
    </div>
  % endif
...
<%def name="host_staticroutes(host)">
  % if len(host.get('static_routes', {}).items()) != 0:
    
    <h4>Static Routes</h4>
    <div>
      <ul>
       % for item_index in range(0, (len(host.get('static_routes')['stdout_lines']))):
           <li class="liSR">${host.get('static_routes')['stdout_lines'][item_index]} </li>
       % endfor
      </ul>    
    </div>
  % endif
...
<%def name="host_listenports(host)">
  % if len(host.get('listen_ports', {}).items()) != 0:
    <h4>Listen Ports</h4>
    <div>
    <ul>
    % for x in range(0, (len(host.get('listen_ports')['stdout_lines']))):
        <li>${host.get('listen_ports')['stdout_lines'][x]}</li>
    % endfor    
    </ul>
    </div>
  % endif
...
<%def name="host_virtualhosts(host)">
  % if len(host.get('virtual_hosts', {}).items()) != 0 and len(host.get('virtual_hosts', {})['stdout_lines']) > 2:
      <h4>Virtual Hosts</h4>
      <div>
      <table>
            <% open=0 %>
            % for x in range(0, (len(host.get('virtual_hosts')['stdout_lines']))):
                % if host.get('virtual_hosts')['stdout_lines'][x] == '/etc/nginx/vhosts/':
                    % if open == 0:
                        <b>Root vhosts:</b>
                    ...
                    % else:
                        ...
                        <b>Root vhosts:</b>
                    % endif
                % elif host.get('virtual_hosts')['stdout_lines'][x] == '/var/containers/nginx/etc/nginx/vhosts/':
                    % if open == 0:
                        <b>Dockerized vhosts:</b>
                    ...
                    % endif
                % else:
                    <li class="liVH">${host.get('virtual_hosts')['stdout_lines'][x]}</li>
                % endif
            % endfor 
